...
export default function usePhoneContactsApiConsumer() {
    ...
    const getPhoneContacts = async () => {
        try {
            ...
            const mappedData = response.data.map((item: { id: string; name: string; country: string; city: string; avatar: string; address: string; accounts: Account[]; calls: Account[]; }) => mapDataToPhoneContact(item));
    ...
    const filterData = (query: string) => {
        ...
        contacts.forEach((contact: PhoneContact) => {
            if (query === "" || contact.getName().includes(query) || contact.getCountry().includes(query) || contact.getAddress().includes(query) || contact.getCity().includes(query)) {
